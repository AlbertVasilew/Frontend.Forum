...
const Menu = () => {
    ...
    useEffect(() => {
        setCategoriesMenus(categoryContext.categories.map(category =>
            ({ ...category, path: `/category/${category.id}` })));
    }, [categoryContext.categories])

    useEffect(() => {
        setMenus(menus => menus.map(menu =>({
            ...menu, count: primaryMenuContext.primaryMenuCounters[menu.name.toLowerCase()]})));
                ...
                <Box className="drawer-menu">
                    ...
                    <MenuList>
                        {menus?.map(menu => (
                            <MenuItem
                                key={menu.name}
                                className={`drawer-menu-item ${isActive(menu.path) ? `active` : ''}`}
                    ...
                    <MenuList>
                        {categoriesMenus.map(categoryMenu => (
                            <MenuItem
                                ...
                                className={`drawer-menu-item ${isActive(categoryMenu.path) ? `active` : ''}`}
                                onClick={() => navigate(categoryMenu.path)}
                            >
                                <ListItemIcon>
                        ...
                        <MenuItem className="drawer-menu-item" onClick={() => setManageCategoryDialog(true)}>
                            <ListItemIcon>
